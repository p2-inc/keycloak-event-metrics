...
public class KeycloakExtension implements BeforeAllCallback, ParameterResolver {
  ...
  public void beforeAll(ExtensionContext context) throws Exception {
    ...
    var container =
        new GenericContainer<>("quay.io/keycloak/keycloak:" + version)
            ...
            .withCopyFileToContainer(
                MountableFile.forHostPath(jar), "/opt/keycloak/providers/metrics.jar")
    ...
    assertEquals(
        version, keycloak.serverInfo().getInfo().getSystemInfo().getVersion(), "version invalid");
    var target = ClientBuilder.newClient().target(url);
    var token = Keycloak.getClientProvider().targetProxy(target, TokenService.class);
    prometheus =
        new Prometheus(Keycloak.getClientProvider().targetProxy(target, PrometheusClient.class));
  ...
  public boolean supportsParameter(
    ...
    return Set.of(KeycloakClient.class, Prometheus.class)
  ...
  public Object resolveParameter(
      ParameterContext parameterContext, ExtensionContext extensionContext) {
    return parameterContext.getParameter().getType().equals(KeycloakClient.class)
        ? client
        : prometheus;
  }
}
